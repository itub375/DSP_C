...
void extract_features(float32_t* mag, float32_t* mag_past, AudioFeatures* features)
{
    ...
    // Peak Frequency index und Amplitude
    ...
    features->spectral_flatness_past = features->spectral_flatness;
    ...
    // Features in Struct AudioFeatures schreiben
...
int main()
{
    ...
    while(true)
    {
        ...
        if(frame_count % 20 == 0)
        {
            // IF_DEBUG(debug_printf("SC: %.2f Hz\n", left_features.spectral_centroid));
            // IF_DEBUG(debug_printf("P: %.2f dB\n", left_features.power));
             IF_DEBUG(debug_printf("FC: %.2f dB\n", flux_counter));
