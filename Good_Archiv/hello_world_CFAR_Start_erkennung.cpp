...
#define CFAR_THRESHOLD_FACTOR 6.0f  // Schwellwert-Faktor (anpassbar)
...
int count = 0;
int index_count=50;  // Warum 50 ? weils funktioniert (unter 20 gehts nicht)
...
int main()
{
    ...
    while(true)
    {
        ...
        if(signal_detected == true && count < index_count){
        ...
        if(frame_count % 2 == 0)
        {
           ...
           // IF_DEBUG(debug_printf("Score: %.2f\n", score));
            ...
            if (score > score_threshold and count == index_count)
            {
                IF_DEBUG(debug_printf("Score: %.2f\n", score));
