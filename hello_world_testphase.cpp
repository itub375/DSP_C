...
// Hier nur diese Werte anpassen
    ...
    uint16_t weightCentroid = 3;        // 2
    uint16_t weightPower = 2;           // 2
    uint16_t weightEnergy = 1;          // 2
    uint16_t weightpeakF = 2;           // 1
    uint16_t weightFlux = 2;            // 3
    uint16_t weightbandwidth = 3;          // 2
    uint16_t weightRolloff = 1;           // 1
    uint16_t weightFlatness = 0;            // 3

    float32_t score = 0.0f;             // 0
    float32_t score_threshold = 0.6f;   // 0.3

    uint16_t threshCentroid = 638;      // 200
    uint16_t threshPower = 4;           // 3
    uint16_t threshEnergy = 5;          // 5
    uint16_t threshpeakF = 1733;        // 300
    uint16_t threshFlux = 1;            // 2
    uint16_t threshBandwidth = 1000;     // 5
    uint16_t threshRolloff = 800;       // 300
    uint16_t threshFlatness = 0.3;      // 2
...
int main()
{
    ...
    // Step 8: Erstmaliges einlesen von Signal A
        ...
        if(signal_detected == true && Signal_count < min_Signal_count){
            ...
            IF_DEBUG(debug_printf("RO: %.2f dB\n", signalA.spectral_rolloff));
    ...
    // Step 12 Detection of Signal A
        ...
        if(fabs(left_features.spectral_bandwidth - signalA.spectral_bandwidth) < threshBandwidth) {
            score += weightpeakF*(1.0f-normalize(fabs(left_features.spectral_bandwidth- signalA.spectral_bandwidth), 0, threshBandwidth));
        }

        // Peak Rolloff
        if(fabs(left_features.spectral_rolloff - signalA.spectral_rolloff) < threshRolloff) {
            score += weightpeakF*(1.0f-normalize(fabs(left_features.spectral_rolloff- signalA.spectral_rolloff), 0, threshRolloff));
        }

        // Peak Flatness
        if(fabs(left_features.spectral_flatness - signalA.spectral_flatness) < threshFlatness) {
            score += weightpeakF*(1.0f-normalize(fabs(left_features.spectral_flatness- signalA.spectral_flatness), 0, threshFlatness));
        }
