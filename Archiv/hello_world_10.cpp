...
// FFT-Parameter
#define FFT_SIZE 256              // 5.33ms @ 96kHz
#define FFT_HOP_SIZE 128          // 50% Overlap
...
// Feature-Historie
#define FEATURE_HISTORY_SIZE 100  // ~5 Sekunden bei 50% Overlap
...
void print_memory_usage(void) {
    debug_printf("\n=== MEMORY USAGE ===\n");
    debug_printf("input_ring:       %lu bytes\n", sizeof(state.input_ring));
    debug_printf("audio_buffer:     %lu bytes\n", sizeof(state.audio_buffer));
    debug_printf("feature_history:  %lu bytes\n", sizeof(state.feature_history));
    debug_printf("active_segments:  %lu bytes\n", sizeof(state.active_segments));
    debug_printf("Total state:      %lu bytes\n", sizeof(state));
    debug_printf("FFT buffers:      %lu bytes\n", 
                sizeof(fft_input) + sizeof(fft_output) + sizeof(spectrum_magnitude));
    debug_printf("Other buffers:    %lu bytes\n",
                sizeof(in) + sizeof(out) + sizeof(left_in) + sizeof(right_in) + 
                sizeof(left_out) + sizeof(right_out));
    debug_printf("====================\n\n");
}
#endif
...
int main()
{
    ...
    debug_printf("%s, %s\n", __DATE__, __TIME__);
    IF_DEBUG(debug_printf("=== REAL-TIME AUDIO DEINTERLEAVING 2 ===\n"));
