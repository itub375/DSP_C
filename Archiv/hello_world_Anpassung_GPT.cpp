...
#define W_SUM (W_CENTROID + W_BANDWIDTH + W_FLATNESS + W_ROLLOFF + \
               W_POWER + W_ENERGY + W_PEAKFREQ + W_FLUX)

// Schwellwerte
#define TH_CENTROID   300.0f    // Hz
#define TH_BANDWIDTH  500.0f    // Hz
#define TH_FLATNESS   0.30f
#define TH_ROLLOFF    800.0f    // Hz
#define TH_POWER     3.0f       // dB
#define TH_ENERGY    5.0f
#define TH_PEAKFREQ  300.0f     // Hz
#define TH_FLUX      2.0f


float32_t score_threshold = 0.40f;
float32_t score = 0.0f;             // 0
...
void extract_features_opt(const float32_t* mag,
                          const float32_t* mag_past,
                          AudioFeatures* f)
{
    ...
    float32_t sum_freq = 0.0f;
    float32_t sum_pow = 0.0f;
    ...
    /* Backup past */
    f->spectral_centroid_past = f->spectral_centroid;
    f->spectral_bandwidth_past = f->spectral_bandwidth;
    f->spectral_flatness_past = f->spectral_flatness;
    f->spectral_rolloff_past = f->spectral_rolloff;
    f->energy_past = f->energy;
    f->power_past = f->power;
    f->peak_frequency_past = f->peak_frequency;
    f->peak_magnitude_past = f->peak_magnitude;
    f->flux_past = f->flux;

    for(uint32_t i = 1; i < K; i++)
    {
        float32_t m = mag[i] + EPS;
        float32_t f_i = (float32_t)i * df;
    ...
    float32_t centroid = sum_freq / (sum_mag + EPS);
    f->spectral_centroid = centroid;

    for(uint32_t i = 1; i < K; i++)
    {
        float32_t diff = (float32_t)i * df - centroid;
        sum_bw += diff * diff * mag[i];
    }
    f->spectral_bandwidth = sqrtf(sum_bw / (sum_mag + EPS));

    float32_t geo = expf(sum_log * invK);
    float32_t arith = sum_mag * invK;
    f->spectral_flatness = geo / (arith + EPS);

    float32_t thr = 0.85f * sum_mag;
    float32_t acc = 0.0f;
    f->spectral_rolloff = fs * 0.5f;

    for(uint32_t i = 1; i < K; i++)
    {
        acc += mag[i];
        if(acc >= thr) {
            f->spectral_rolloff = (float32_t)i * df;
            break;
        }
    }

    f->energy = sum_pow;
    f->power  = 10.0f * log10f(sum_pow * invK);

    f->peak_frequency = (float32_t)peak_idx * df;
    f->peak_magnitude = peak_mag;
    f->flux = flux;
}


static inline float32_t norm_diff(float32_t diff, float32_t thresh)
{
    if(diff >= thresh) return 0.0f;
...
float32_t compute_score(const AudioFeatures* cur,
                        const AudioFeatures* ref)
{
    float32_t score = 0.0f;

    score += W_CENTROID * norm_diff(fabsf(cur->spectral_centroid - ref->spectral_centroid),TH_CENTROID);

    score += W_BANDWIDTH * norm_diff(fabsf(cur->spectral_bandwidth - ref->spectral_bandwidth),TH_BANDWIDTH);

    score += W_FLATNESS * norm_diff(fabsf(cur->spectral_flatness - ref->spectral_flatness),TH_FLATNESS);

    score += W_ROLLOFF * norm_diff(fabsf(cur->spectral_rolloff - ref->spectral_rolloff),TH_ROLLOFF);

    score += W_POWER * norm_diff(fabsf(cur->power - ref->power),TH_POWER);

    score += W_ENERGY * norm_diff(fabsf(cur->energy - ref->energy),TH_ENERGY);

    score += W_PEAKFREQ * norm_diff(fabsf(cur->peak_frequency - ref->peak_frequency),TH_PEAKFREQ);

    score += W_FLUX * norm_diff(fabsf(cur->flux ),TH_FLUX);

    return score / (float32_t)W_SUM;
}
...
int main()
{
    ...
    // Step 6: Extract features from left channel
        extract_features_opt(magnitude, magnitude_past, &left_features);
    ...
    // Step 12 Detection of Signal A
        ...
        score = compute_score(&left_features, &signalA);
    ...
    // Step 13: Output features (example - every 100th frame)
        ...
        if(frame_count % 100 == 0)
        {
            ...
            IF_DEBUG(debug_printf(
                "BW %.2f FL %.2f RO %.2f F %.2f | SCORE %.2f\n",
