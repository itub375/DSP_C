...
int main()
{
    ...
    IF_DEBUG(debug_printf("Streaming started. Press BUTTON_A to cycle clusters.\n"));
    IF_DEBUG(debug_printf("Current cluster: %d\n", state.target_cluster));
    ...
    while(true)
    {
        ...
        // Button-Handling: Cluster-Auswahl
        if(TRUE) {
            if(button_debounce_counter == 0) {
                state.target_cluster = (state.target_cluster + 1) % state.num_clusters;
                state.fade_gain = 0.0f;  // Reset Fade beim Wechsel
                button_debounce_counter = 96000 / BLOCK_SIZE / 2;  // 500ms Debounce
                
                IF_DEBUG(debug_printf("Switched to cluster: %d\n", state.target_cluster));
            }
        }
